...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return (
    (rect1Right > rect2.left && rect1Bottom > rect2.top)
  ...
  // return (
  //   rect1.left < rect2.len && rect1.len > rect2.left
  //   && rect1.top < rect2.rightBot && rect1.rightBot > rect2.top
  // );
}
...
function isCreditCardNumber(ccn) {
  const numArr = Array.from(String(ccn), Number).reverse();
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  numStr = numStr.reduce((acc, curr) => acc + curr, 0);
  return numStr > 10 ? getDigitalRoot(numStr) : numStr;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) {
      ...
      if (pairBrackets[str[i]] === stack[stack.length - 1]) {
        stack.pop();
      } else {
        return false;
      }
